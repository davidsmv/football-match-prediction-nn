...
- **`src/services/fbref_data_scraper.py`** — Core scraper using `undetected-chromedriver` + Selenium to bypass FBRef's bot detection. Renders JavaScript-heavy pages, then parses fixture tables with BeautifulSoup/lxml. Outputs a DataFrame with columns: week, day, date, time, home, score, away, attendance, venue, referee, match_report.
- **`src/services/notebooks/`** — Jupyter notebooks for data exploration (`data.ipynb` loads scraped CSVs, `soccerdata.ipynb` experiments with the `soccerdata` library).
- **`src/data/`** — Scraped output (CSV files, raw HTML). Not the source of truth — regenerated by running the scraper.
- **`src/main.py`** — Entry point (currently empty, intended for orchestration).
